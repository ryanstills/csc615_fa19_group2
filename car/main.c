...
void * getSonar(void * carInfo){ // get the measurement results of ultrasonic module, with unit: cm
    ...
    while(car->mode != 0){
	    ...
	    readout = (float)pingTime * 340.0 / 2.0 / 1000.0;
...
int main(){
	...
	if(carInfo->mode == 1){
		...
		left_motor_speed = setMotorSpeed(LEFT_MOTOR, 0, HIGH, LOW);
		...
		right_motor_speed = setMotorSpeed(RIGHT_MOTOR, 0, HIGH, LOW);
		usleep(2500);
	    }
	    ...
	    left_motor_speed = setMotorSpeed(LEFT_MOTOR, left_motor_speed , LOW, HIGH);
	    right_motor_speed = setMotorSpeed(RIGHT_MOTOR, right_motor_speed, LOW, HIGH);
	}
	if(count > 6000){
	    carInfo->mode = 0;
	    printf("+++++++++++++++++++++++++++++++++++++++++++++\n");
